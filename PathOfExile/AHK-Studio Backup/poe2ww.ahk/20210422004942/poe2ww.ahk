...
global GTP:=0,toggle:=0,bloodragetick_s:=0,bloodragetick_c:=0,RBtickS:=0,RBtickC:=0,XB1tickS:=0,XB1tickC:=0,EtickS:=0,EtickC:=0,CRunTickS:=99999,CRunTickC:=0
...
$~*RButton::RBtickS:=A_TickCount
$~*XButton1::XB1tickS:=A_TickCount
$~*e::EtickS:=A_TickCount


SendSomething:
RBtickC:=A_TickCount - RBtickS
XB1tickC:=A_TickCount - XB1tickS
EtickC:=A_TickCount - EtickS
;CRunTickC:=A_TickCount - CRunTickS
if (XB1tickC<300)
{
	sleep 190
	send {F7}
...
Toggle(onn)	{
	...
	if onn = 0
	{
		...
		;Hotkey, *~C, off
		send {blind}{shift up}{LButton up}{RButton up}{t up}{r up}
		SoundBeep, 1000, 50
	}
	return
}
