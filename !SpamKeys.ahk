...
Global sequenceTime := 0
Global tog:=0,toggle:=0,time_since_last:=0,time_key_s:=0,time_key_c:=0
Global XButton1:="XButton1",XButton2:="XButton2",LButton:="LButton",RButton:="RButton",MButton:="MButton",Shift:="Shift",Ctrl:="Ctrl",Alt:="Alt"
global keySequence := []
global keyIndex := 1
global keyDelay := delay
global sound_on := A_WinDir . "\Media\Speech Sleep.wav"
global sound_off := A_WinDir . "\Media\Speech On.wav"
...
Spam2(ifkey1,skey1,to1:=5,ifkey2:="",skey2:="",ifkey3:="",skey3:="",ifkey4:="",skey4:="",holdkey1:=5) {
	WinGet, sATitle, ProcessName, A
	while ((GetKeyState(ifkey1, "P")) and (WinActive("ahk_exe "sATitle)) and (!GetKeyState("Alt", "P")))
	{
...
TogTimerF(kk1:="",kk2:="",kk3:="",kk4:="",kk5:="",to:=5,ik1:="",ik2:="",ik3:="",ik4:="",ik5:="",holdkey1:=5,holdkey2:=5) { ;{
		global tog, gATitle, gk1, gk2, gik1, gik2, gholdkey1, gholdkey2, gto		
		if (kk1 = "")
...
TogTimerF2(kk, ik, to, holdKey, seq) { ;{
		if (!kk)
			kk := Array(RegExReplace(A_ThisHotkey, "\W", ""))
		if (!ik)
			ik := Array()
		if (!holdKey)
			holdKey := Array()
		...
		global gk:=kk, gik:=ik, gholdKey:=holdKey, gto:=to, gATitle:="", gseq:=seq, gseqIndex:=1, sequenceTime:=0
		
		WinGet, gATitle, ProcessName, A
		if isRunning {
			SetTimer, TogSpamTimer2, off
			isRunning := False
			TogSpamTimerRun := 0
			...
			SetTimer, TogSpamTimer2, %to%
			isRunning := True
			TogSpamTimerRun := 1
			SoundPlay, %A_WinDir%\Media\Speech On.wav
			sequenceTime := A_TickCount
		}
	}
	TogSpamTimer2: 
	global gk, gik, gholdKey, gto, gATitle, gseq, gseqIndex, sequenceTime
	if WinActive("ahk_exe " gATitle) {
		if (gseq.length() > 0 && Mod(gseq.length(), 2) = 0) {
			if (A_TickCount > sequenceTime + gseq[gseqIndex + 1]) {
				keyToSend := gseq[gseqIndex]
				send {blind}{%keyToSend% down}
				sleep 50 ; delay to ensure the key press registers
				send {blind}{%keyToSend% up}
				sequenceTime := A_TickCount
				if (gseqIndex + 2 > gseq.length()) {
					gseqIndex := 1
				}
				else {
					gseqIndex += 2
		...
		for index, key in gk {
			if (gik[index] != "" && gik[index] != 0 && GetKeyState(gik[index], "P")) {
				send {blind}{%key% down}
				sleep gholdKey[index]
				send {blind}{%key% up}
			}
			else if (gik[index] = "" || gik[index] = 0) { 
				send {blind}{%key% down}
				sleep gholdKey[index]
				send {blind}{%key% up}
			}
...
return ;}
TogTimerF3(kk, delays, hold, seq) { ;{
		global keyList, delayList, IsRunning, holdDelay, seqList, seqDelayList, lastKeySendTimes, lastSeqSendTimes, lastSeqSendTime, gATitle
		...
		keyList := kk, delayList := delays, holdDelay := hold, seqList := [], seqDelayList := []
		
		for index, val in seq {
			if (Mod(index, 2) == 1)
				seqList.Push(val)
			else
				seqDelayList.Push(val)
		}
		
		for index, key in keyList {
			lastKeySendTimes[key] := A_TickCount - delays[index]
			msgbox, % lastKeySendTimes[key]
		}
		
		for index, key in seqList {
			if !lastSeqSendTimes.HasKey(key)
				lastSeqSendTimes[key] := A_TickCount
		}
		
		lastSeqSendTime := A_TickCount
		
		WinGet, gATitle, ProcessName, A
		if !IsRunning {
			
			SetTimer, TogSpamTimer3, 5
			IsRunning := True
			SoundPlay, %A_WinDir%\Media\Speech On.wav
		}
		else if IsRunning {
			SetTimer, TogSpamTimer3, Off
			IsRunning := False
			SoundPlay, %A_WinDir%\Media\Speech Sleep.wav
			send {blind}{alt up}{shift up}{ctrl up}
		}
	...
	TogSpamTimer3:
	global keyList, delayList, IsRunning, holdDelay, seqList, seqDelayList, lastKeySendTimes, lastSeqSendTimes, lastSeqSendTime, nextSeqIndex, gATitle
	
	if WinActive("ahk_exe " gATitle) {
		for index, key in keyList {
			
			if (A_TickCount - lastKeySendTimes[key] > delayList[index]) {
				sendKey(key)
				lastKeySendTimes[key] := A_TickCount
			}
		}
		
		if (A_TickCount - lastSeqSendTime > seqDelayList[nextSeqIndex]) {
			sendKey(seqList[nextSeqIndex + 1])
			lastSeqSendTimes[seqList[nextSeqIndex + 1]] := A_TickCount
			lastSeqSendTime := A_TickCount
			nextSeqIndex := Mod(nextSeqIndex + 1, seqList.length())
		}
	}
return ;}
sendKey(skey){
...
SpamTimerF(kk1:="",kk2:="",kk3:="",kk4:="",kk5:="",to:=5) { ;{
		kk1 := % RegExReplace(A_ThisHotkey, "\W", "")
		global gkA := [kk1, %kk2%, %kk3%, %kk4%, %kk5%] 
		global gto=%to%
		WinGet, gATitle, ProcessName, A
...
Clipboard := "#IfWinActive ahk_exe "currwin "`t`t`t`t`t`t`t`t`t`t" ";{ ["wintitl "]" "`n`n #IF " ";}"
KeyWait Ctrl, D
KeyWait v, D
Clipboard := Ctmp
return
$~^S::			;{ [AHK Studio - D:\Install\SystemFolders\Progs\!my\Scripts\!SpamKeys.ahk]
	DetectHiddenWindows, ON
	if WinActive("ahk_class AutoHotkeyGUI")
	{
		...
		{
			Run *RunAs %A_ScriptDir%\%scriptname%.ahk
			ExitApp
		}
	}
	DetectHiddenWindows, Off
...
#IfWinActive ahk_exe VRising.exe										;{ [VRising]
	$XButton2::Spam2(XButton2,"f",5)
#IF ;}
#IfWinActive ahk_exe TheDivision2.exe										;{ [Tom Clancy's The Division 2]
...
#IfWinActive ahk_exe Returnal-Win64-Shipping.exe										;{ [Returnal  ]
	XButton2::TogTimerF("LButton","Shift",,,,6,"LButton","Shift")
#IF ;}
#IfWinActive ahk_exe Diablo IV.exe										;{ [Diablo IV]
	XButton2::TogTimerF("LButton",,,,,6,"LButton")
#IF ;}
#IfWinActive ahk_exe DrugDealerSimulator-Win64-Shipping.exe		;{
	*$c::
	...
	;MouseClickDrag, Left, %prev_x%,%prev_y%, 1553, 496, 2 ;1533, 675
	;send {blind}{alt up}
	;MouseMove,%prev_x%,%prev_y%,0
	;return
	...
	*$XButton1::
	while GetKeyState("XButton1","P")
	{
		...
		if  GetKeyState("RButton","P")
		{
			...
			MouseGetPos,prev_x,prev_y
			sleep 1
			;MouseClick,Left,971, 869
			sleep 1
			send {blind}{alt down}
			sleep 20
			;MouseClickDrag, Left, %prev_x%,%prev_y%, 1553, 496, 2 ;1533, 675 ;971, 869 ;962, 888
			send {blind}{LButton down}
			sleep 1
			MouseMove, 20,20,2,R
			MouseMove, 1553, 496, 0
			sleep 1
			send {blind}{LButton up}
			sleep 1
			MouseMove, %prev_x%,%prev_y%, 0
	...
	MouseClick, Left, 1556, 721,1,0
	;MouseMove,%prev_x%,%prev_y%,0
	;MouseMove,960,540,0 ;center
	return
...
#IfWinActive ahk_exe Last_Epoch.exe										;{ []
	$*RButton::Spam2(RButton,LButton,25,LButton,"A",XButton2,"Q",,,10)
	$*XButton1::
	send {shift down}
	Spam2(XButton1,RButton)
	send {shift up}
	return
	;$~*RButton::Spam2("RButton","a",25)
#IF ;}
#IfWinActive ahk_exe GSS2-Win64-Shipping.exe										;{ []
	$XButton2::Spam2(XButton2,LButton,5)
#IF ;}
#IfWinActive ahk_exe Cyberpunk2077.exe						;{ [Cyberpunk 2077 (C) 2020 by CD Projekt RED]
	$XButton2::Spam2(XButton2,XButton2,25,XButton1,XButton1,XButton2,Shift,XButton2,XButton2)
	;$XButton1::Spam()
	;$*XButton1::Spam("XButton1")
#IF ;}

#IfWinActive ahk_exe WarmSnow.exe										;{ [WarmSnow]
	$XButton2::Spam2(XButton2,Space,10,XButton2,LButton)
	$Space::Spam()
#IF ;}
#IfWinActive ahk_exe Palworld-Win64-Shipping.exe										;{ [Pal  ]
	*XButton2::Spam2("XButton2","F")
#IF ;}

#IfWinActive BlazblueEntropyEffect										;{ [BlazblueEntropyEffect]
;$*MButton::TogTimerF2("XButton1","XButton2",,,,15,,,,,,20,20)
	
	$XButton1::Spam2("XButton1","k",5,"XButton1","j",,,,,15)
	
#IF ;}

#IfWinActive ahk_exe Big Ambitions.exe										;{ [Big Ambitions]
	XButton2::Enter
#IF ;}


#IfWinActive ahk_exe DDDA.exe										;{ [Dragon’s Dogma: Dark Arisen]
	E::Spam()
#IF ;}

#IfWinActive ahk_exe (the) Gnorp Apologue.exe										;{ [(the) Gnorp Apologue]
;XButton2::Spam2("XButton2","LButton",1)
	XButton2::togfunc(3,"spamLB",1)
	spamLB:
	send {LButton down}
	sleep 5
	send {LButton up}
	return
#IF ;}
#IfWinActive ahk_exe Archon-Win64-Shipping.exe									;{ [Dungeons Of Sundaria  ]
	$F::Spam()
	$XButton1::Spam()
#IF ;}

#IfWinActive ahk_exe Dungeons-Win64-Shipping.exe				;{ [Minecraft Dungeons  ]
;MButton::TogTimerF2(["LButton", "3"], [5, 3050], ["1", "2"], [9500, 9500])
#IF ;}
#IfWinActive ahk_exe Dungeons.exe								;{ [Minecraft Dungeons  ]
	MButton::TogTimerF2(["LButton","3"],[],15,5,["1",9500,"2",9500]) 
...
#IfWinActive ahk_class SDL_app							;{ [VN pron games]
	...
	*~XButton1::CTRL
	#IF
	/*
		#IfWinActive ahk_exe EmberKnights_64.exe					;{ [EmberKnights]
			*XButton2::Spam2("XButton2","LButton",5,"XButton2","Space")
			RButton::Spam2("RButton","LButton",5,,,,,,,800)
			Space::Spam()
		#IF ;}
		#IfWinActive ahk_exe Rogue Legacy 2.exe						;{ [Rogue Legacy 2]
			$~*RButton::Spam()
			$~*WheelDown::send e
			$~*WheelUp::send q
		#IF ;}
		...
		#IfWinActive ahk_exe The Slormancer.exe						;{ [The Slormancer]
			~$*RButton::Space
			XButton1::TogTimerF("e",,,,,20,0)
		#IF ;}
		#IfWinActive ahk_exe VampireSurvivors.exe					;{ [Vampire Survivors]
			PgDn::send x-x1viiq
		#IF ;}
		#IfWinActive ahk_exe r2g_launcher.exe						;{ [Book of Demons]
			XButton1::Spam2("XButton1","LButton",5,"XButton1","RButton")
		#IF ;}
		#IfWinActive ahk_exe D2R.exe								;{ [Diablo II: Resurrected] 
			$*~b::send {RButton down}
			$*~b up::send {RButton up}w
			$*~q::send {RButton down}
			$*~q up::send {RButton up}w
			$*~a::send {RButton down}
			$*~a up::send {RButton up}w
			$*~RButton::send x
			$*~RButton up::send c
			$*XButton1::TogTimerF("LButton","4","RButton",,,10,0,0,"RButton")
...
;$*Space::Spam()
		...
		#IfWinActive ahk_exe LoR.exe								;{ [Legends of Runeterra]
			$*RButton::Spam2("RButton","LButton",10)
		#IF ;}
...
;Return Pattern } }
		#If ;}
		/*#IfWinActive ahk_exe chrome.exe 							;{ Number sender to phone, LFJ WORK (outdated)
;F2::
...
;$*~XButton1::Spam2("XButton1","XButton1",3,"Shift","Shift")
			...
			#IF ;}
			#IfWinActive ahk_exe Gunfire Reborn.exe							;{ [Gunfire Reborn]
				$*Space::Spam2("Space","Space",10,"XButton1","XButton1")
				$*XButton2::Spam2("XButton2","XButton1",10)
;$~*LButton::Spam("LButton")
;$*XButton2::Spam2("XButton2","XButton2",10,"Space","Space")
				$CapsLock::return
				$*XButton1::TogTimerF("XButton1",,,,,,0,0) ;"Space"
...
;if (xpos not between 1770 and 1854) and (ypos not between 1163 and 1238)
			...
			#IfWinActive ahk_class SDL_app							;{ [VN pron games]
				...
				*~XButton2::Spam2("XButton2","LButton")
				#IF
;#IfWinActive ahk_class UnityWndClass	; [MY New Born Life In A Harem]
;*RButton::Spam2("RButton","LButton")
...
;$*~LButton::Spam("LButton")
			...
			#IfWinActive ahk_exe Diablo III64.exe						;{
				~$*MButton::TogTimerF("F12","LButton",,,,10,0,"LButton")
;$~*LButton::Shift
			#IF ;}
