...
Spam2(ifkey1,skey1,to1:=5,ifkey2:="",skey2:="",ifkey3:="",skey3:="",ifkey4:="",skey4:="",holdkey1:=5) {
	...
	{
		if ((GetKeyState(ifkey1, "P")) and ((ifkey2="") and (skey2="")))
		{
			send {blind}{%skey1% down}
			sleep %holdkey1%
			send {blind}{%skey1% up}
		}
		else	if ((GetKeyState(ifkey2, "P")) and ((ifkey3="") and (skey3="")))
		{
			sleep to1
		...
		else	if ((GetKeyState(ifkey3, "P")) and ((ifkey4="") and (skey4="")))
			{
				sleep to1
		...
		else	if ((GetKeyState(ifkey4, "P")) and ((ifkey5="") and (skey5="")))
			{
				sleep to1
...
#IfWinActive ahk_exe OUTRIDERS-Win64-Shipping.exe		;*[Outriders] ;{
	$~*w::F7
	$~*s::F7
	$~*RButton::Spam2("RButton","LButton",100)
	$~*XButton1::v
	$~*Space::
