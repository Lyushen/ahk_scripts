...
#IfWinActive ahk_exe MiniHealer.exe						;{
	RButton::Spam2("RButton","LButton",5)
	$*q::Spam()
	$*z::Spam()
	$*x::Spam()
	$*a::Spam()
	...
	return
	1::
	preMposX:="",preMposY:="",rPixColor
	MouseGetPos, preMposX, preMposY
	waitPixClick("0x564637",1451,779,10) ;1459, 810,5) ;564637
	waitPixClick("0xBD97CF",1662,729,1,,2,1451,779) ;1672, 760,1,,2,1459, 810) ;564637
	/*
		PixelGetColor, rPixColor, 1672, 760, RGB ;9F77D0
		if !rPixColor = 0x9F77D0
		{
			msgbox, "aren't 5 gems"
			MouseClick, Left,
		}
	*/
	...
	*/
	...
	{
    ; Default the custom click coordinates to the pixel-check coordinates if not provided
		if (cwx = 0)
			cwx := x
		if (cwy = 0)
			cwy := y
    ...
    ; Wait for the pixel color to match and then click at (x,y)
		if (waitAndClick = 1)
		{
    ...
    ; Wait for the pixel color to match and then click at (cwx,cwy)
		if (waitAndClick = 2)
		{
			while (cPixColor != rPixColor)
			{
    ...
    ; Click at x,y while the pixel color (cwx,cwy) matches
		if (waitAndClick = 3)
		{
			while (cPixColor = rPixColor)
			{
