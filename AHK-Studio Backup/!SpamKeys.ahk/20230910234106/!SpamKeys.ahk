...
SoundBeep, 1000, 50
Run, *RunAs D:\Install\SystemFolders\Progs\Tools\nircmd\nircmdc.exe setappvolume /%ScriptPID% 0.02,, hide
SetBatchLines, -1
SetKeyDelay, -1, -1
SetWinDelay, -1 
SetControlDelay, -1
...
Clipboard := "#IfWinActive ahk_exe "currwin "`t`t`t`t`t`t`t`t" ";{ ["wintitl "]" "`n`n #IF " ";}"
KeyWait Ctrl, D
KeyWait v, D
Clipboard := Ctmp
DetectHiddenWindows, OFF
...
#IfWinActive ahk_exe Grim Dawn.exe						;{ [Grim Dawn]
	$~XButton1::TogTimerF("LButton","MButton",,,,10,0,0)
#IF ;}
...
#IfWinActive ahk_exe Dungeons-Win64-Shipping.exe				;{ [Minecraft Dungeons  ]
	MButton::TogTimerF("LButton","3",,,,15,0,0) ;,"XButton1","1","XButton1","Space"
#IF ;}
...
#IfWinActive ahk_exe MiniHealer.exe						;{
	MButton::Spam2("MButton","LButton",5)
	$*q::Spam()
	$*z::Spam()
	$*x::Spam()
	$*a::Spam()
	...
	PixelGetColor, mposColor, mposX, mposY, RGB
	clipboard := mposColor . " " . mposX . "," . mposY
	return
	F5:: ;test pixel color
	xy:=[1672,760]
	PixelGetColor, rPixColor, xy[1], xy[2], RGB ;9F77D0	
	MouseMove, xy[1], xy[2], 1
	;msgbox %rPixColor%
	; Display the color in a GUI overlay
	Gui, Add, Text,, Pixel Color: %rPixColor%
	Gui, +AlwaysOnTop +LastFound
	WinSet, Transparent, 150 ; Setting it less than 255 makes it a bit transparent
	Gui, Show, NA, OverlayTitle
	return
	2::executeSequence("0x564737", 627, 778, 2)
	
	1:: 
	executeSequence(
    "0x564637", 1451, 779, 7, 0, 1, 1451, 779,
    "0xBD97CF", 1662, 729, 1, 0, 2, 1451, 779,
    "0x564637", 1466, 440, 1, 0, 1, 1466, 440,
    "0x31271C", 922, 968, 2, 0, 1, 922, 968,
    "0x31271C", 922, 968, 1, 0, 3, 922, 968
	)
	
	executeSequence(ParamList*) {
		MouseGetPos, preMposX, preMposY
		for index, value in ParamList {
			if (Mod(index-1,8) = 0) ; Every 8 values we have a new sequence
				waitPixClick(value, ParamList[index+1], ParamList[index+2], ParamList[index+3], ParamList[index+4], ParamList[index+5], ParamList[index+6], ParamList[index+7])
		}
		MouseMove, preMposX, preMposY, 0
		return
	}
	
	waitPixClick(rPixColor, x, y, clicks:=1, speed:=0, waitAndClick:=1, cwx:=x, cwy:=y) {
		PixelGetColor, cPixColor, x, y, RGB
		while (true) {
			if ((waitAndClick = 1 && cPixColor = rPixColor) || (waitAndClick = 3 && cPixColor != rPixColor))
				break
			else if (waitAndClick = 2 && cPixColor = rPixColor) {
				MouseClick, Left, cwx, cwy, %clicks%, %speed%
				return
			}
			Sleep, 5
			PixelGetColor, cPixColor, x, y, RGB
	...
	/*
		2::
		MouseGetPos, preMposX, preMposY
		waitPixClick("0x564737",627,778,2)
		MouseMove, preMposX, preMposY, 0
		return
		
		1::
		MouseGetPos, preMposX, preMposY
		waitPixClick("0x564637",1451,779,7) ;1459, 810,5) ;564637
		waitPixClick("0xBD97CF",1662,729,1,,2,1451,779) ;1672, 760,1,,2,1459, 810) ;564637
		waitPixClick("0x564637",1466, 440) ;564637 ;1474, 471)
		waitPixClick("0x31271C",922, 968,2) ;38100D 930, 999)
		waitPixClick("0x31271C",922, 968,1,,3)
		MouseMove, preMposX, preMposY, 0
		return
		
		waitPixClick(rPixColor, x, y, clicks:=1, speed:=0, waitAndClick:=1, cwx:=0, cwy:=0)
		{
			if (cwx = 0) ; Default the custom click coordinates to the pixel-check coordinates if not provided
				cwx := x
			if (cwy = 0)
				cwy := y
			PixelGetColor, cPixColor, x, y, RGB
			if (waitAndClick = 1) ; Wait for the pixel color to match and then click at (x,y)
			{
				while (cPixColor != rPixColor)
				{
					Sleep, 1
					PixelGetColor, cPixColor, x, y, RGB
				}
			...
			if (waitAndClick = 2) ; Wait for the pixel color to match and then click at (cwx,cwy)
			{
				while (cPixColor != rPixColor)
				{
					Sleep, 5
			...
			if (waitAndClick = 3) ; Click at x,y while the pixel color (cwx,cwy) matches
			{
				while (cPixColor = rPixColor)
				{
					Sleep, 5
