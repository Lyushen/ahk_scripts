...
global TSOkey1:="",TSOholdto:=5,TSOto:=5,QLB:=0,QRB:=0,QXB1:=0,Qq:=0,Qw:=0,Qe:=0,Qa:=0,Qs:=0,Qd:=0
...
#IfWinActive ahk_exe Wolcen.exe ;{
	...
	if tog
	{
		...
		SoundPlay, %A_WinDir%\Media\Speech Sleep.wav
		sleep 100
		send {blind}{alt up}{shift up}{ctrl up}{LButton up}{LButton up}{F7 up}
	}
	else if !tog
	{
	...
	{
		send {LButton up}
		while (!GetKeyState("d","p") or (!GetKeyState("ESC","p")))
		{
			sleep 5
		}
