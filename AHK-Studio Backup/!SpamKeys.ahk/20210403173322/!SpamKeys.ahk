...
global tog:=0,TogSpamTimerRun:=0
global time_since_last:=0
global ttog:=0
global gk1:=0,gk2:=0,gk3:=0,gk4:=0,gk5:=0,gk6:=0,gk7:=0,gk8:=0,gto:=5,gATitle:="",gik1:=0,gik2:=0,gik3:=0,gik4:=0,gik5:=0,gik6:=0,gik7:=0,gik8:=0
...
Spam(key1:="",key2:="",key3:="",to1:=5,to4:=5,to2:=40,to3:=40) {
...
Spam2(skey1,ifkey1:="",to1:=5,ifkey2:="",skey2:="",ifkey3:="",skey3:="",ifkey4:="",skey4:="") {
	WinGet, sATitle, ProcessName, A
	;if (trigerkey="")
	trigerkey := % RegExReplace(A_ThisHotkey, "\W", "")
	while ((GetKeyState(trigerkey, "P")) and (WinActive("ahk_exe "sATitle)))
	{
		if (!GetKeyState("ALT", "P"))
		{
			if ((trigerkey!=ifkey1) and (GetKeyState(ifkey1, "P")))
			{
				send {blind}{%skey1% down}
				sleep 5
				send {blind}{%skey1% up}
			}
			if ((trigerkey=ifkey1) or (ifkey1=""))
			{
				send {blind}{%skey1% down}
				sleep 5
				send {blind}{%skey1% up}
...
TogSpamTimerFunc(kk1:="",kk2:="",kk3:="",kk4:="",kk5:="",to:=5,ik1:="",ik2:="",ik3:="",ik4:="",ik5:="") {
	if (kk1="")
...
if (WinActive("ahk_exe "gATitle)) {
	...
	if (GetKeyState(gik2, "P")) {
		send {blind}{%gk2% down}
		sleep 5
		send {blind}{%gk2% up}
	}
	if (GetKeyState(gik3, "P")) {
		send {blind}{%gk3% down}
		sleep 5
		send {blind}{%gk3% up}
	}
...
#IfWinActive ahk_exe OUTRIDERS-Win64-Shipping.exe		;*[Outriders] ;{
	$~*w::F7
	$~*XButton1::v
	Shift::SendInput,% "{F7 "((toggle:=!toggle)?"Down":"Up")"}" and "{w "((toggle:=!toggle)?"Down":"Up")"}"
	;Shift::
	;send {blind}{F7 up}{F7 down}
	;$~*LButton::
...
$XButton2::TogSpamTimerFunc("f","e",,,,15,"w","XButton1")
$*XButton1::
if (!tog)
	Spam2("e")
return
