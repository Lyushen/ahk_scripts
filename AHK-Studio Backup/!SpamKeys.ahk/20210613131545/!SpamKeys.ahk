...
global TSOkey1:="",TSOholdto:=5,TSOto:=5,QLB:=0,QRB:=0,QXB1:=0,Qq:=0,Qw:=0,Qe:=0,Qa:=0,Qs:=0
...
#IfWinActive ahk_exe Wolcen.exe ;{
	...
	testt:
	if (time_key_c:=A_TickCount-time_key_s > 1000)
	MsgBox
	return
	...
	if tog
	{
		...
		tog:=!tog
		SoundPlay, %A_WinDir%\Media\Speech Sleep.wav
		send {blind}{alt up}{shift up}{ctrl up}{F6 up}
	}
	else if !tog
	{
	...
	if (!GetKeyState("F6","D") && (!GetKeyState("LButton","P"))) 
	{
		send {F6 down}
	}
	;if (GetKeyState("LButton","D") && (!GetKeyState("LButton","P"))) 
	time_key_c := A_TickCount - time_key_s
	MsgBox %time_key_c% , %time_key_s%
	if ((time_key_c>1500) and (QLB=1))
	{
