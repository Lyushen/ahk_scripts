...
global TSOkey1:="",TSOholdto:=5,TSOto:=5,lbq:=0,rbq:=0,XB1q:=0,qq:=0,wq:=0,eq:=0,aq:=0,sq:=0
...
;~$*XButton2::togfunc(3,"wolcen_main_tick",5,"timer_wolcen_s1",100)
	...
	if tog
	{
		SetTimer, wolcenn, off
		tog:=!tog
		SoundPlay, %A_WinDir%\Media\Speech Sleep.wav
		send {blind}{alt up}{shift up}{ctrl up}{LButton up}
	}
	else if !tog
	{
		SetTimer, wolcenn, 30
	...
	wolcenn:
	if ((qq=1) or (wq=1) or (eq=1) or (aq=1) or (sq=1))
	{
		sleep 50
		qq:=0
		wq:=0
		...
		aq:=0
		sq:=0
		KeyWait % RegExReplace(A_ThisHotkey, "\W", ""), D
	}
	;if (lbq=1)
	;{
		;send {RButton down}
